...
export const stellarRouter = createTRPCRouter({
  ...
  kycUploadFiles: publicProcedure
    .input(
      z.object({
        type: z.string(), // "sender" | "receiver"
        transferId: z.string(),
        photoIdFront: z.string().optional(), // Base64 encoded file
        photoIdFrontType: z.string().optional(), // Content type
        photoIdBack: z.string().optional(), // Base64 encoded file
        photoIdBackType: z.string().optional(), // Content type
        sep12Id: z.string().optional(), // KYC ID
      }),
    )
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        // Create a mock transfer if one doesn't exist (for development)
        if (!transfer) {
          console.log(`Transfer with ID ${input.transferId} not found, creating a mock transfer record`);
          transfer = await ctx.db.transfer.create({
            data: {
        ...
        // Determine auth session ID based on type
        let authSessionId = null;
        if (input.type === "receiver") {
          authSessionId = transfer.receiverAuthSessionId;
        } else {
          authSessionId = transfer.senderAuthSessionId;
        }

        if (!authSessionId) {
          throw new TRPCError({
            message: "Auth session not linked to transfer",
            code: "BAD_REQUEST",
          });
        }
        ...
        // Find or create KYC entry
        let kycEntry = await ctx.db.kYC.findFirst({
          where: {
            authSessionId: authSessionId,
            userId: authSession.userId,
          },
        ...
        if (!kycEntry) {
          // Use provided sep12Id or generate a new one
          const sep12Id = input.sep12Id || `local-kyc-${Date.now()}`;
          
          kycEntry = await ctx.db.kYC.create({
            data: {
              userId: authSession.userId,
              authSessionId: authSession.id,
              sep12Id: sep12Id,
        ...
        // Process front ID photo if provided
        if (input.photoIdFront) {
          try {
            // Extract the base64 data (remove data:image/jpeg;base64, prefix)
            const base64Data = input.photoIdFront.split(',')[1] || input.photoIdFront;
            
            // Convert base64 to Buffer
            const photoBuffer = Buffer.from(base64Data, 'base64');
            
            // Add to update data
            updateData.photo_id_front = photoBuffer;
            updateData.photo_id_front_type = input.photoIdFrontType || 'image/jpeg';
          } catch (error) {
            console.error('Error processing front ID photo:', error);
          }
        }
        
        // Process back ID photo if provided
        if (input.photoIdBack) {
          try {
            // Extract the base64 data (remove data:image/jpeg;base64, prefix)
            const base64Data = input.photoIdBack.split(',')[1] || input.photoIdBack;
            
            // Convert base64 to Buffer
            const photoBuffer = Buffer.from(base64Data, 'base64');
            
            // Add to update data
            updateData.photo_id_back = photoBuffer;
            updateData.photo_id_back_type = input.photoIdBackType || 'image/jpeg';
          } catch (error) {
            console.error('Error processing back ID photo:', error);
          }
        }
        
        // Only update if we have data to update
        if (Object.keys(updateData).length > 0) {
          await ctx.db.kYC.update({
            where: { id: kycEntry.id },
            data: updateData,
          });
        }
        
        return { success: true, kycId: kycEntry.sep12Id };
      } catch (e) {
        ...
        throw new TRPCError({
          message: "Failed to save KYC documents",
          code: "INTERNAL_SERVER_ERROR",
          cause: e,
        });
      }
