  ...
  useEffect(() => {
    if (!isLoading) {
      if (user && user.walletAddress) {
        // Redirect to the user's investments page using their wallet address
        router.push(`/dashboard/${user.walletAddress}/investments`);
      } else if (user) {
        // User is authenticated but doesn't have a wallet address yet
        // Try using user.id for routing
        router.push(`/dashboard/${user.id}/investments`);
      } else {
